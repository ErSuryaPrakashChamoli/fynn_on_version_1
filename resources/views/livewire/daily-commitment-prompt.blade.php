{{--
    ...
    the wire:model binding it was first initialised with — typing into the
    OTP box would still be writing to `amount`, and the button in the
    footer would still be calling the previous branch's method. The keys
    are what force a replacement instead of a reuse; do not remove them.
--}}
@php
        ...
        >
            <div class="w-full max-w-3xl rounded-xl bg-white shadow-2xl ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10">
                ...
                @if ($reason === \App\Services\DailyCommitmentGate::REASON_COMMIT)
                    <div wire:key="prompt-commit" class="border-b border-gray-200 px-6 py-5 dark:border-white/10">
                    ...
                    <div wire:key="prompt-commit-fields" class="space-y-4 px-6 py-5">
                        <label wire:key="commit-stage" class="block">
                            ...
                            >
                                @foreach ($stageOptions as $value => $label)
                                    {{-- @selected, not left to Alpine: until it boots the
                                         browser shows option[0], which is a different stage
                                         from the one the server is holding. --}}
                        ...
                        @if ($stage === \App\Enums\CommitmentStage::Otp->value)
                            <label wire:key="commit-count" class="block">
                        ...
                        @else
                            <label wire:key="commit-amount" class="block">
                    ...
                    <div wire:key="prompt-commit-footer" class="flex items-center justify-between gap-3 border-t border-gray-200 px-6 py-4 dark:border-white/10">
                        <span class="text-xs text-gray-500 dark:text-gray-400">
                            Due by {{ \App\Services\DailyCommitmentGate::MORNING_DEADLINE }} every day.
                        </span>
                        <x-filament::button wire:click="giveCommitment" icon="heroicon-o-check">
                            Give commitment
                        </x-filament::button>
                    </div>
                @endif
                ...
                @if ($reason === \App\Services\DailyCommitmentGate::REASON_DECLARE)
                    <div wire:key="prompt-declare" class="border-b border-gray-200 px-6 py-5 dark:border-white/10">
                        <div class="flex items-start gap-3">
                            <x-filament::icon
                    ...
                    @if ($commitment)
                        <div wire:key="prompt-declare-summary" class="flex flex-wrap items-center gap-x-8 gap-y-3 border-b border-gray-200 bg-gray-50 px-6 py-4 dark:border-white/10 dark:bg-white/5">
                            <div>
                    ...
                    @if ($mode === null)
                        <div wire:key="declare-choose" class="grid gap-3 px-6 py-5 sm:grid-cols-2">
                            <button
                                type="button"
                    ...
                    @elseif ($mode === 'cases')
                        <div wire:key="declare-cases" class="max-h-[45vh] space-y-3 overflow-y-auto px-6 py-5">
                            @foreach ($cases as $index => $case)
                                {{-- Keyed on the row: removeCase() re-indexes the array,
                                     so without this the surviving rows keep the bindings
                                     of the rows above them. --}}
                                <div wire:key="case-{{ $index }}" class="grid gap-2 rounded-lg bg-gray-50 p-3 sm:grid-cols-12 dark:bg-white/5">
                                    <input
                                        type="text"
                                    ...
                                    >
                                        <option value="" @selected(blank($case['stage'] ?? null))>Stage…</option>
                        ...
                        <div wire:key="declare-cases-footer" class="flex flex-wrap items-center gap-3 border-t border-gray-200 px-6 py-4 dark:border-white/10">
                    ...
                    @else
                        <div wire:key="declare-failed" class="px-6 py-5">
                            ...
                            <div class="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-5">
                                @foreach (\App\Enums\CommitmentStage::ladder() as $rung)
                                    <label wire:key="nil-{{ $rung->value }}" class="block">
                        ...
                        <div wire:key="declare-failed-footer" class="flex flex-wrap items-center gap-3 border-t border-gray-200 px-6 py-4 dark:border-white/10">
